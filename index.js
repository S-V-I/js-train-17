...
function replaceText(word, replacement, text) {
  ...
  const result = text.replaceAll(new RegExp(word, "g"), replacement);
  return result;
}
...
function checkWord(word, text) {
  // Створення регулярного виразу для пошуку слова з флагом 'i'
  // (регістронезалежний пошук).
  // Використання методу `test` регулярного виразу для перевірки наявності слова
  // у тексті.
  // Повернення результату перевірки.

  const result = new RegExp(word, "i").test(text);
  return result;
}
...
/**
 * Функція `extractTextInParentheses` вилучає текст, який знаходиться
 * в круглих дужках, з рядка.
 *
...
function extractTextInParentheses(str) {
  ...
  return Array.from(str.matchAll(/\((.*?)\)/g), (a) => a[1]);
}
...
// Завдання 5
/**
 * Функція `findWordOccurrences` знаходить всі входження заданого слова у рядок
 * з урахуванням ігнорування регістру.
 *
...
/**
 * Функція `replaceWordOccurrences` замінює всі входження заданого слова у рядку
 * на нове слово.
 *
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  return str.replaceAll(new RegExp(word, "g"), newWord);
}
...
/**
 * Функція `findWord` знаходить перше входження заданого слова у рядок
 * за допомогою методу `search`.
 *
 *  str - Рядок, в якому потрібно знайти входження слова.
 *  word - Слово, входження якого потрібно знайти.
